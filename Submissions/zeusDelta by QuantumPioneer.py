...
def dogde_from_projectile(player_pos, enemy_projectiles):
...
class Script:
    ...
    def combo_a(self, player, enemy):
        if get_stun_duration(enemy) == 0 and get_last_move(player)[0] != 'block':
            ...
            if not heavy_on_cooldown(enemy):
                point += 3
            if get_primary_skill(enemy) == "uppercut" and not primary_on_cooldown(enemy):
                point += 6
    ...
    def strategy_1(self, player, enemy, player_projectiles, enemy_projectiles):
        ...
        if enemy_last_act and enemy_last_act[0] == "grenade":
            return JUMP_FORWARD
        if enemy_projectiles is not None:
            for enemy_projectile in enemy_projectiles:
                dist_danger = abs(player_pos[0] - get_proj_pos(enemy_projectile)[0])
        ...
        if not primary_on_cooldown(player) and (player_hp <= 80 or self.clock > 110):
            ...
            if distance >= 2:
                if distance == 2:
                    if enemy_last_act[0] == 'move':
                        return LIGHT
                    self.count_down = 1
                if enemy_last_act and enemy_last_act[0] == 'onepunch' and get_past_move(enemy, 1)[1] == 'startup':
                    return BACK
                return FORWARD
            elif distance < 2:
                if get_secondary_skill(enemy) == 'boomerang':
                    if not primary_on_cooldown(enemy) and not heavy_on_cooldown(player):
                        return HEAVY
                if enemy_last_act[0] == 'onepunch' and get_past_move(enemy, 1)[1] == 'startup':
                    return JUMP
                if self.count_down:
                    self.count_down -= 1
                    return BLOCK
